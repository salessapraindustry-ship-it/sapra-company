...
# === PRO-FIXER PATCH 20260328_1410 ===
# Fixed: BACKEND_BUILDER
# Issues: generate_backend_code() returns raw LLM JSON without validation, causing parsing failures when model outputs malformed JSON or includes code blocks, deploy_to_github() function is incomplete - cuts off mid-request, causing all deployment operations to fail silently, No error handling for JSON parsing in generate_backend_code() - unescaped quotes, newlines, and special characters in code strings break JSON.loads(), API retry logic exists but isn't used consistently - generate_backend_code() doesn't wrap its API call with _retry_api(), Missing validation that required fields exist in generated JSON before accessing them, No timeout or size limits on generated code, allowing infinite loops or memory exhaustion, The JSON extraction logic using text.index('{') fails when LLM includes explanatory text before JSON
def generate_backend_code(task_description, research_context):
    """Generate complete backend code for a tool."""
    prompt = f"""You are an expert Python backend developer.
Build a complete, deployable backend tool.
...
{{
  ...
  "deployment_cmd": "railway up or render deploy command"
    ...
    try:
        ...
        if not parsed:
            log.error(f"Failed to parse JSON from response. Text preview: {text[:200]}")
            return None
        ...
        if missing:
            log.error(f"Generated tool missing required fields: {missing}")
            return None

        # Validate content is not empty
        if not parsed.get("main_py", "").strip():
            log.error("Generated main_py is empty")
            return None

        log.info(f"  ✅ Generated tool: {parsed.get('tool_name', 'unknown')}")
        return parsed
...
def deploy_to_github(tool_name, files):
    ...
    if not GITHUB_TOKEN or not GITHUB_USERNAME:
        ...
        for filename, content in files.items():
            try:
                (tool_dir / filename).write_text(str(content), encoding='utf-8')
            except Exception as e:
                log.warning(f"Failed to write {filename}: {e}")
        log.info(f"  ✅ Tool saved locally: {tool_dir}")
        return f"local:/tmp/tools/{tool_name}"
    ...
    try:
        ...
        # Git operations
        try:
            subprocess.run(["git", "init"], cwd=tool_dir, check=True, capture_output=True, timeout=10)
            subprocess.run(["git", "config", "user.name", "BackendBuilder"], cwd=tool_dir, check=True, capture_output=True, timeout=10)
            subprocess.run(["git", "config", "user.email", "builder@agent.ai"], cwd=tool_dir, check=True, capture_output=True, timeout=10)
            subprocess.run(["git", "add", "."], cwd=tool_dir, check=True, capture_output=True, timeout=10)
            subprocess.run(["git", "commit", "-m", "Initial commit by BackendBuilder agent"], cwd=tool_dir, check=True, capture_output=True, timeout=10)
            subprocess.run(["git", "branch", "-M", "main"], cwd=tool_dir, check=True, capture_output=True, timeout=10)
            subprocess.run(
                ["git", "remote", "add", "origin", f"https://{GITHUB_TOKEN}@github.com/{GITHUB_USERNAME}/{tool_name}.git"],
                cwd=tool_dir, check=True, capture_output=True, timeout=10
            )
            subprocess.run(["git", "push", "-u", "origin", "main", "--force"], cwd=tool_dir, check=True, capture_output=True, timeout=30)
            log.info(f"  ✅ Deployed to GitHub: {repo_url}")
            return repo_url
        except subprocess.TimeoutExpired:
            log.error("Git operation timed out")
